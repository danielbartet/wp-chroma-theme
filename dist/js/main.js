...
!function (r, t) {
  "object" == (typeof exports === "undefined" ? "undefined" : _typeof(exports)) && "undefined" != typeof module ? t(exports) : "function" == typeof define && define.amd ? define(["exports"], t) : t((r = "undefined" != typeof globalThis ? globalThis : r || self).Redux = {});
}(void 0, function (r) {
  "use strict";
  ...
  function i(r) {
    if ("object" != _typeof(r) || null === r) return !1;
    ...
    function E() {
      ...
      return (r = {
        subscribe: function subscribe(r) {
          if ("object" != _typeof(r) || null === r) throw Error(t(11));
          function e() {
            r.next && r.next(b());
          }
          return e(), {
  ...
  r.__DO_NOT_USE__ActionTypes = o, r.applyMiddleware = function () {
    ...
    if ("object" != _typeof(r) || null === r) throw Error(t(16));
    var n = {};
    for (var o in r) {
      var i = r[o];
...
!function () {
  ...
  function i(t) {
    ...
    if (this.length = 0, t) if ((t = "string" == typeof t ? this.find(t) : t).nodeType || t === t.window) this.length = 1, this[0] = t;else {
      var e = t.length;
  ...
  i.extend = t, i.contains = function (t, e) {
    ...
    addClass: function addClass(t) {
      for (var e, i, s, n = this.length; n;) if ((i = this[--n]).className) for (e = e || t.split(/\s+/), s = 0; s < e.length; s += 1) this.hasClass(e[s]) || (i.className += " " + e[s]);else i.className = t;
      return this;
    },
    removeClass: function removeClass(t) {
      for (var e, i = new RegExp("^(?:" + t.split(/\s+/).join("|") + ")$"), s = /(\S+)(?:\s+|$)/g, n = function n(t, e) {
          return i.test(e) ? "" : t;
  ...
  return f.extend(i.prototype, {
    ...
    updateActiveSlide: function updateActiveSlide(t, e) {
    ...
    handleSlide: function handleSlide(t, e) {
    ...
    imageFactory: function imageFactory(t, e) {
      var i,
        s,
        n,
        o,
    ...
    iteratePreloadRange: function iteratePreloadRange(t, e) {
    ...
    unloadElements: function unloadElements(t, i) {
    ...
    getNestedProperty: function getNestedProperty(o, t) {
  ...
  return C.extend(e, {
    ...
    videoFactory: function videoFactory(t, e, i) {
      var s,
        ...
        v = [p];
      if (h.addClass(l.videoContentClass), C(p).addClass(l.videoPlayClass), C(u).addClass(l.videoCoverClass).hasClass(l.toggleClass) || v.push(u), u.draggable = !1, y && (r.title = y, p.setAttribute("aria-label", y)), g && (u.style.backgroundImage = 'url("' + g + '")'), c.setAttribute ? l.videoPlaysInline && c.setAttribute("playsinline", "") : h.addClass(l.videoIframeClass), c.preload = this.getItemProperty(t, l.videoPreloadProperty) || "none", this.support.source && f) for (o = 0; o < f.length; o += 1) c.appendChild(C.extend(this.sourcePrototype.cloneNode(!1), f[o]));
...
function doesExist(el) {
  if (el !== null && typeof el !== 'undefined') return true;
}
"use strict";
...
function inView(element) {
  ...
  return elementSize.top >= 0 && elementSize.left >= -100 && elementSize.bottom <= 620 + (window.innerHeight || html.clientHeight) && elementSize.right <= 2640 + (window.innerWidth || html.clientWidth);
}
...
var _facebookApiInit = _interopRequireDefault(require("/var/www/html/wp-content/plugins/cm-components/components/forms/signup/facebook-api-init.js"));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }
document.addEventListener("DOMContentLoaded", function () {
  'use strict';

  var fbApiInit = new _facebookApiInit["default"]();
  function chromaFormHandler() {
    ...
    function submitForm(formIdVal) {
      var type = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'subscribe';
      ...
      fetch(postURL, formOptions).then(function (response) {
        return response.text();
      }).then(function (text) {
        var msg = text;
        if (formSuccess(msg) && callBack !== null) {
          var _duplicateEmail = msg.split(',')[0] === '["Already Subscribed!"' ? true : false;
          callBack(_duplicateEmail);
        }
      })["catch"](function (error) {
        return console.log('Form submission error: ' + error);
      });
    }
    this.facebookSignup = function (el) {
      FB.api("/me", "GET", {
        ...
        var t = "https://" + window.location.hostname + "/form-processing/",
          ...
          type = 'fblogin';
        var dataBody = "&email=".concat(e.email, "&type=fblogin&currURL=").concat(n);
        var formOptions = {
          method: 'post',
          headers: {
            'Content-type': 'application/x-www-form-urlencoded; charset=UTF-8'
        ...
        fetch(postURL, formOptions).then(function (response) {
          ...
          var msg = text;
          if (formSuccess(msg) && callBack !== null) {
            var _duplicateEmail2 = msg.split(',')[0] === '["Already Subscribed!"' ? true : false;
            callBack(_duplicateEmail2);
          }
          if (el.getAttribute('data-next') !== null && el.getAttribute('data-next').indexOf('http') !== -1) window.location = el.getAttribute('data-next');
        })["catch"](function (error) {
          alert('Facebook Login Error. Please try again later.');
          console.log(e);
        });
      });
    };
    function formSuccess(text) {
      ...
      if (Array.isArray(text)) {
        for (var _i = 0, _l = text.length; _i < _l; _i++) {
          text[_i] = text[_i].replace(/"/g, "");
          text[_i] = text[_i].replace(/]/g, "");
          text[_i] = text[_i].replace(/\[/g, "");
        }
        var _errorMSG = document.getElementById('errorMessage'),
          _errorMSG2 = document.getElementById('errorMessage2');
        _errorMSG.classList.remove('is-active');
        _errorMSG.innerText = text[0];
        _errorMSG2.innerText = text[1];
        _errorMSG.classList.add('is-active');
      } else {
        text = text.replace(/"/g, "");
        var _errorMSG3 = document.getElementById('errorMessage');
        _errorMSG3.classList.remove('is-active');
        _errorMSG3.innerText = text;
        _errorMSG3.classList.add('is-active');
      }
      return true;
    }
    this.init = function () {
      ...
      try {
        if (doesExist(document.getElementById('subscribe'))) {
          ...
          subscriber.addEventListener('submit', function (event) {
            event.preventDefault();
            //gather context data
        ...
        if (document.getElementsByClassName('fb-arrow').length > 0) {
          [].forEach.call(document.getElementsByClassName('fb-arrow'), function (el) {
  ...
  var formHandler = new chromaFormHandler();
  formHandler.init();
});
var _default = exports["default"] = chromaFormHandler;
"use strict";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var scrollAnchors = /*#__PURE__*/function () {
    ...
    return _createClass(scrollAnchors, [{
      key: "init",
      value: function init() {
        var _this = this;
...
document.getElementById('c-wp-menu-toggle').addEventListener('click', function () {
  document.getElementsByClassName('c-wp-col-left')[0].classList.toggle('menu-expand');
});
"use strict";

(function () {
  var heart = document.querySelector('.post-like .heart'),
    // Retrieve post ID from data attribute
    postId = typeof postVar != 'undefined' ? postVar : null;
  if (postId === null || !doesExist(heart)) return;
  ...
  fetch(ajaxPage.url, {
    ...
    body: "action=post_like_update&post_id=".concat(postId)
  }).then(function (response) {
    console.log(response);
    return response.text().then(function (text) {
      heart.classList.add('voted');
      console.log(text);
      var userLiked = text.split(":");
    ...
    return console.log('Success: ', response.status);
  })["catch"](function (error) {
  ...
  heart.addEventListener('click', function (ev) {
    fetch(ajaxPage.url, {
      ...
      body: "action=post-like&nonce=".concat(ajaxPage.nonce, "&post_like=&post_id=").concat(postId)
    }).then(function (response) {
      return response.text().then(function (text) {
        if (text != 'already') {
          heart.classList.add('voted');
